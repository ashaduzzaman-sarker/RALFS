...
from ralfs.evaluation.faithfulness import EntityGrid, compute_egf
from ralfs.evaluation.main import EvaluationResult, RALFSEvaluator, compare_systems
from ralfs.evaluation.metrics import evaluate_bertscore, evaluate_rouge
...
class TestEvaluatorWithConfidenceIntervals:
    ...
    def test_confidence_intervals(self):
        ...
        predictions = [{"id": f"test{i}", "summary": f"Summary {i}"} for i in range(20)]
        references = [{"id": f"test{i}", "summary": f"Reference {i}"} for i in range(20)]
...
class TestEvaluator:
    ...
    def test_evaluate_single_sample(self):
        ...
        predictions = [{"id": "test1", "summary": "The cat sat on the mat."}]
        references = [{"id": "test1", "summary": "The cat was on the mat."}]
    ...
    def test_evaluate_multiple_samples(self):
        ...
        predictions = [{"id": f"test{i}", "summary": f"Summary {i}"} for i in range(5)]
        references = [{"id": f"test{i}", "summary": f"Reference {i}"} for i in range(5)]
