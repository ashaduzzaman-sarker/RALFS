...
class TestFiDDataset:
    ...
    def test_dataset_initialization(self, tmp_path):
        ...
        with open(data_file, "w") as f:
            for i in range(5):
                sample = {"text": f"Sample text {i}", "metadata": {"summary": f"Summary {i}"}}
    ...
    def test_dataset_getitem(self, tmp_path):
        ...
        with open(data_file, "w") as f:
            sample = {"text": "Test document text", "metadata": {"summary": "Test summary"}}
            f.write(json.dumps(sample) + "\n")
    ...
    def test_create_dataloader(self, tmp_path):
        ...
        with open(data_file, "w") as f:
            for i in range(10):
                f.write(
                    json.dumps({"text": f"Text {i}", "metadata": {"summary": f"Summary {i}"}})
                    + "\n"
                )
