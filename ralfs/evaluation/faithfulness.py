...
class EntityGrid:
    ...
    def _build_grid(self) -> dict[str, list[str]]:
        ...
        for sent_idx, sent in enumerate(self.sentences):
            ...
            for mention in mentions:
                ...
                if current_role == "-" or self._role_priority(mention.role) > self._role_priority(
                    current_role
                ):
    ...
    def _compute_transitions(self) -> dict[str, int]:
        ...
        for _entity, roles in self.grid.items():
    ...
    def get_transition_probabilities(self) -> dict[str, float]:
        ...
        return {trans: count / total for trans, count in self.transitions.items()}
...
class EntityGridFaithfulness:
    ...
    COHERENT_TRANSITIONS = {
        "SS",
        "SO",
        "SX",  # Subject continuity (strong coherence)
        "OS",
        "OO",  # Object continuity (moderate coherence)
    }
    ...
    INCOHERENT_TRANSITIONS = {
        ...
        "-S",
        "-O",
        "-X",  # Abrupt introductions
        "S-",
        "O-",
        "X-",  # Abrupt disappearances
    }
    ...
    def _coherence_score(self, grid: EntityGrid) -> float:
        ...
        coherent = sum(grid.transitions.get(t, 0) for t in self.COHERENT_TRANSITIONS)
        incoherent = sum(grid.transitions.get(t, 0) for t in self.INCOHERENT_TRANSITIONS)
