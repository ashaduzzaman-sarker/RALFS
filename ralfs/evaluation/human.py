...
def create_human_eval_template(
    ...
    logger.info(
        "  Consistency (1-5): How faithful is the summary to the source (no hallucinations)?"
    )
...
def create_pairwise_comparison_template(
    ...
    common_ids = list(set(pred_a_dict.keys()) & set(pred_b_dict.keys()) & set(ref_dict.keys()))
