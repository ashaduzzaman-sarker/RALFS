...
class HybridRetriever(BaseRetriever):
    ...
    def retrieve(
        ...
        if reranked_results:
            ...
            for result, norm_score in zip(reranked_results, normalized, strict=False):
    ...
    def _linear_fusion(
        ...
        colbert_results: list[RetrievalResult] | None = None,
    ) -> list[RetrievalResult]:
        """
        ...
        # Add dense scores
        for result, norm_score in zip(dense_results, dense_scores, strict=False):
            text = result.text
            fused_scores[text] = self.alpha * norm_score
            text_to_result[text] = result

        # Add sparse scores
        for result, norm_score in zip(sparse_results, sparse_scores, strict=False):
            text = result.text
        ...
        if colbert_results:
            ...
            for result, norm_score in zip(colbert_results, colbert_scores, strict=False):
                text = result.text
                if text in fused_scores:
