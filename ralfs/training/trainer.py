...
class RALFSTrainer:
    ...
    def setup_optimizer(self, train_dataloader: DataLoader):
        ...
        learning_rate = getattr(model_cfg, 'learning_rate', 5e-5) if model_cfg else getattr(training_cfg, 'learning_rate', 5e-5)
        weight_decay = getattr(model_cfg, 'weight_decay', 0.01) if model_cfg else getattr(training_cfg, 'weight_decay', 0.01)
    ...
    def train(
        ...
        eval_dataset: Optional[FiDDataset] = None,
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(f"  Effective batch size: {batch_size * gradient_accumulation_steps * self.accelerator.num_processes}")
        ...
        for epoch in range(num_epochs):
            ...
            for step, batch in enumerate(progress_bar):
                ...
                # Log
                if self.global_step % logging_steps == 0 and (step + 1) % gradient_accumulation_steps == 0:
                ...
                # Save checkpoint
                if self.global_step % save_steps == 0 and (step + 1) % gradient_accumulation_steps == 0:
                    self.save_checkpoint(f"step_{self.global_step}")
                
                # Evaluate
                if eval_dataset and self.global_step % eval_steps == 0 and (step + 1) % gradient_accumulation_steps == 0:
                    ...
                    current_metric = eval_metrics.get('rouge', {}).get(metric_for_best, eval_metrics['loss'])
