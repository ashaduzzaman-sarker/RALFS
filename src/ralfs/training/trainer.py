...
from ralfs.core.constants import CHECKPOINTS_DIR
...
def train(cfg):
    ...
    for epoch in range(cfg.model.epochs):
        ...
        accelerator.save_state(f"{CHECKPOINTS_DIR}/epoch_{epoch+1}")

            # 3. Save
        CHECKPOINTS_DIR.mkdir(parents=True, exist_ok=True)
        out_path = CHECKPOINTS_DIR / f"{dataset_name}_chunks.jsonl"
...
def train(cfg):
    ...
    logger.info("Training complete!")
    # accelerator.save_state(f"{cfg.training.output_dir}/final")
    accelerator.save_state(f"{CHECKPOINTS_DIR}/final")
